...
while True:
    ...
    #Threding
    #cv2.putText(frame, plates_list[ind], (30,60),cv2.FONT_HERSHEY_SIMPLEX,2, (255,0,0),5,)
...
#Kada naidje tablica slikaj i zapamti
    ...
    try:
        ...
        #print(ind)
        print(' '+str(plates_list[ind]))

        if str(plates_list[ind]) in str(data):
            # if GPIO==True:
            #     print('Nista ne radi')
            # else:
            #     Ovde kod
            if otvori1==15:
                print("Otvori rampu za tablice:", str(plates_list[ind]))
                #print(str(data[0]))

                time.sleep(5)
                

                with open('Vrijeme_nailaska-ulaz.txt','a+') as text:
                    text.write(str(plates_list[ind]))
            ...
            #print("Nisi ovlasten da pristupis:", str(plates_list[ind]))
            print(' -------------------- ')
            #tablica(str(plates_list[ind]))
        continue
...
if str(plates_list) in open('Vrijeme.txt').read():
        Tablica(str(plates_list))
"""
i = 0
for plate in results['results']:
    i += 1
